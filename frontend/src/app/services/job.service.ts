...
const DEFAULT_PRINTERS: PrinterOption[] = [
  {
    id: "PRUSA_XL_1",
    name: "Prusa XL (Dual Tool)",
    displayName: "Prusa XL (Dual Tool)",
    model: "PRUSA_XL",
    status: "IDLE",
    currentMaterial: "PLA",
    currentColour: "Prusa Orange",
  },
  {
    id: "PRUSA_MK4S_1",
    name: "Prusa MK4S #1",
    displayName: "Prusa MK4S #1",
    model: "PRUSA_MK4S",
    status: "IDLE",
    currentMaterial: "PETG",
    currentColour: "Galaxy Black",
  },
  {
    id: "PRUSA_CORE_ONE_1",
    name: "Prusa Core One #1",
    displayName: "Prusa Core One #1",
    model: "PRUSA_CORE_ONE",
    status: "IDLE",
    currentMaterial: "PLA",
    currentColour: "White",
  },
...
export class JobService {
  ...
  getPrinters(): Observable<PrinterOption[]> {
    return this.http
      .get<PrinterOption[]>(`${API_BASE}/printers`)
      .pipe(catchError(() => of(DEFAULT_PRINTERS)));
  }

  validateGcode(
    file: File,
    printerId: string,
  ): Observable<GcodeUploadResponse> {
    const formData = new FormData();
    formData.append("file", file);
    formData.append("printerId", printerId);
    return this.http.post<GcodeUploadResponse>(
      `${API_BASE}/gcode/upload`,
      formData,
    );
  }
  ...
  pauseJob(jobId: number): Observable<Job> {
    return this.http.post<Job>(`${API_BASE}/jobs/${jobId}/pause`, {});
  }

  resumeJob(jobId: number): Observable<Job> {
